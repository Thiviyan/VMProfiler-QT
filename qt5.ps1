...
Copy-Item "$tools_folder\qtbinpatcher.*" "$prefix_folder\bin\"
Copy-Item "$openssl_bin_folder\*.dll" "$prefix_folder\bin\"
Copy-Item "$openssl_bin_folder\*.exe" "$prefix_folder\bin\"
Copy-Item "$openssl_bin_folder\*.pdb" "$prefix_folder\bin\"
Copy-Item "$openssl_libs_folder\*" "$prefix_folder\lib\" -Recurse
Copy-Item "$openssl_include_folder\openssl" "$prefix_folder\include\" -Recurse

Copy-Item "$mysql_lib_folder\libmysql.*" "$prefix_folder\bin\"
Copy-Item "$mysql_lib_folder\libmysql.*" "$prefix_folder\lib\"
...
Get-ChildItem -r -include "*.prl" $prefix_folder | foreach-object { $a = $_.fullname; (get-content $a).Replace($openssl_libs_folder_esc, '$$$$[QT_INSTALL_LIBS]\\') | set-content $a }

Get-ChildItem -r -include "*.prl" $prefix_folder | foreach-object { $a = $_.fullname; (get-content $a).Replace($mysql_lib_folder_esc, '$$$$[QT_INSTALL_LIBS]\\') | set-content $a }
